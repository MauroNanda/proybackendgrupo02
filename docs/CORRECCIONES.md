...
| C-02 | Auth | ✅ **RESUELTO** (T-11): registro fuerza rol ASISTENTE | **Alta** | No | `services/auth.service.js` registro |
| C-03 | Auth | ✅ **RESUELTO** (T-11): default `false` + login/OAuth manejan 2FA | Media | No | `models/usuario.model.js` + `services/auth.service.js` |
| C-04 | Inscripciones | ✅ **RESUELTO**: `inscribirse` valida estado (PUBLICADO) y fecha futura | Media | No | `services/inscripcion.service.js` inscribirse |
| C-05 | Auth | ✅ **RESUELTO** (C-08): unique constraint → 409 vía error-handler | Media | No | `services/auth.service.js:15-34` |
| C-06 | Eventos | ✅ **RESUELTO**: catálogo público solo lista PUBLICADO | Media | No | `services/evento.service.js` listar |
...
| C-08 | Backend | ✅ **RESUELTO**: error-handler mapea errores Sequelize (409/400) | Media | No | `middlewares/error-handler.middleware.js` |
...
| C-16 | Dashboard | ✅ **RESUELTO**: KPI `totalEventos` cuenta solo PUBLICADO | Baja | No | `services/dashboard.service.js` |
