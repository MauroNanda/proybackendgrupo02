...
4. **El navegador pide permiso** — `Notification.requestPermission()`. Si el usuario no acepta ("granted"), el flujo se corta con un error.
...
4. **Por qué llega con la pestaña cerrada:** el mensaje no va a la página, va al service worker (`public/sw.js`), que el navegador despierta cuando el servicio push recibe algo. `event.waitUntil(showNotification(...))` evita que el SW se duerma antes de mostrarla.
...
- **Las claves VAPID se generan una sola vez** (por ejemplo con `npx web-push generate-vapid-keys`) y quedan fijas en el `.env`. La clave pública que usa el front (`environment.vapidPublicKey` o la que devuelve `GET /api/push/vapid-public-key`) tiene que ser **la misma** que la del backend: el navegador ata cada suscripción a la clave con la que se creó, así que si se regeneran las claves, todas las suscripciones existentes dejan de servir y los usuarios tienen que volver a suscribirse.
- **HTTPS obligatorio.** Los service workers y `PushManager` solo existen en contextos seguros: HTTPS o `localhost`. Por eso funciona en desarrollo local y en el hosting de producción (que ya sirve HTTPS), pero no funcionaría en un HTTP plano — el navegador directamente no expone la API.
- **Sin claves VAPID el push queda desactivado**, no roto: `asegurarVapid()` loguea un warning y `enviarPush` retorna sin enviar; `GET /api/push/vapid-public-key` responde **503** ("Web Push no está configurado en el servidor") y el front le muestra el error al usuario que intenta activar las alertas. El resto de la app y los otros canales (in-app, email) siguen funcionando.
- **La suscripción es por navegador y dispositivo**, no por cuenta: Chrome en la PC y Chrome en el celular generan endpoints distintos, y ambos quedan asociados al mismo `usuario_id` (por eso `enviarATodasDelUsuario` recorre todas). Si una suscripción expira o el usuario revoca el permiso, el próximo envío devuelve **410** y el canal borra esa fila — la tabla se limpia sola, sin tarea de mantenimiento.
- **Es un canal más del sistema de notificaciones.** Push no decide *cuándo* avisar: el hub `integrations/notificaciones.js` recibe el aviso del servicio de negocio y lo reparte a in-app, email y push. Si mañana se quita el push, los otros canales siguen igual; si se agrega otro canal, los servicios de negocio no se tocan.
